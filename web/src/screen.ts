...
/** What the client knows about the space it has. */
export interface Viewport {
  /** CSS pixels available for the grid. */
  width: number
  /** CSS pixels available. */
  height: number
  /** Width of one character cell, measured — never assumed. */
  cellWidth: number
  /** Height of one line. */
  cellHeight: number
}

/** A grid size to ask the instance for. */
 ...
 */
export function fit(viewport: Viewport): Fit {
 ...
 * displayed.
 */
export function worthResizing(current: Fit, next: Fit): boolean {
  return current.cols !== next.cols || current.rows !== next.rows
}

/** The font size that would fit a target column count. */
export function sizeForColumns(width: number, columns: number, advanceRatio = 0.6): number {
 ...
 */
export function isLineOriented(alternateScreen: boolean): boolean {
  return !alternateScreen
}

/** Trailing blank rows carry no information and cost vertical space. */
export function trimTrailingBlank(rows: readonly string[]): string[] {
  const out = [...rows]
