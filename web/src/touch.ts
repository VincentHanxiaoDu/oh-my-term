...
export interface BarKey {
  /** What it shows. */
  label: string
  /** What it sends, or the modifier it latches. */
  action: { send: string } | { latch: 'ctrl' | 'alt' }
}
...
export const KEY_BAR_SECONDARY: readonly BarKey[] = [
...
export function applyLatch(latched: 'ctrl' | 'alt' | null, key: string): string {
...
/** A gesture the terminal understands. */
export type Gesture =
  | { kind: 'arrows'; direction: 'up' | 'down' | 'left' | 'right'; repeat: number }
  | { kind: 'tab' }
  | { kind: 'scroll'; lines: number }
  | { kind: 'resize'; scale: number }
  | { kind: 'switch'; delta: number }
 ...
 */
export function dragToArrows(dx: number, dy: number): Gesture | null {
...
export function gestureBytes(gesture: Gesture): string {
  switch (gesture.kind) {
    case 'arrows': {
...
/**
 * How dangerous an action is, which decides how hard it is to confirm.
 */
export type Risk = 'ordinary' | 'reversible' | 'irreversible'
 ...
 */
export function confirmation(risk: Risk): 'tap' | 'hold' {
...
export function approvalStillValid(
  approved: { id: string; commandHash: string },
  current: { id: string; commandHash: string },
): boolean {
 ...
 */
export function riskOf(command: string): Risk {
