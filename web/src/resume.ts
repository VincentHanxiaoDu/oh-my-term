...
import { type OmtEvent, type Seq, scopeKey } from './protocol.js'

/** What to do about an event that arrived. */
export type Accepted =
  | { action: 'deliver' }
  | { action: 'skip'; why: 'already-seen' }
  | { action: 'gap'; expected: Seq; got: Seq }

/** One client's position in every stream it follows. */
export class ResumeState {
  #positions = new Map<string, Seq>()

  /** Where this client is in a scope, if it has seen anything. */
  position(key: string): Seq | undefined {
    return this.#positions.get(key)
  }

  /** The resume map to send on reconnect. */
  since(): Record<string, Seq> {
   ...
   */
  inspect(event: OmtEvent): Accepted {
