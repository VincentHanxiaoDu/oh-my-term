...
export interface ConnectOptions {
  /** The instance's base URL, e.g. `https://box.local:7717`. */
  url: string
...
/** A minimal transport the client drives. */
export interface Transport {
  send(message: ClientMessage): void
  close(): void
}

/** What a caller supplies to be driven. */
export interface Handlers {
  onMessage(message: ServerMessage): void
  onOpen?(): void
  onClose?(code: number, willRetry: boolean): void
}
...
export function connect(
  ...
  handlers: Handlers,
  factory: (url: string, protocols: string[]) => WebSocket = (u, p) => new WebSocket(u, p),
): Transport {
