...
| `Seq` | [03 §4](03-capability-catalog.md#4-events-are-the-read-side-twin) | Monotonic sequence number, **per session** (or per workspace for workspace-scoped events). Terminal bytes share the session's space ([07 §5.1](07-remote-protocol.md#51-sequence-spaces)). **`u64` everywhere, including on the wire** ([07 §3.6](07-remote-protocol.md#36-binary-payloads)) — it never wraps, and a decrease means a sequence reset, not a wrap. Never sort by `ts`. |
...
| `CapabilityRegistry` / dispatch | [03 §3](03-capability-catalog.md#35-the-dispatch-path) | `name → ErasedHandler`; the single dispatch path. Authorization happens here, not in a transport, so no surface can bypass it. |
| `CallContext` | [03 §3](03-capability-catalog.md#35-the-dispatch-path) | `{ actor, role, request_id, deadline }` carried into every handler. |
...
| Capability error codes | [03 §3](03-capability-catalog.md#35-the-dispatch-path) | Closed enum: `not_found`, `conflict`, `unauthorized`, `precondition_failed`, `unsupported`, `internal`. Protocol adds `unsupported_proto`, `auth_failed`, `rate_limited`, `frame_too_large` ([07 §3.5](07-remote-protocol.md#35-capability-call--result)). Clients switch on `code`, never on `message`. |
...
| `InteractionState` | [06 §5](06-agent-layer.md#5-interactions--the-flagship-path) | `Open \| Resolving { by, response } \| Submitted { by, response, at } \| Resolved \| Undelivered { reason, response } \| Cancelled \| Abandoned`. `Submitted` means the bytes were written; **`Resolved` requires omt to have *observed* the agent record the answer** ([D15](decisions.md#d15--five-classes-of-pending-intent-each-with-its-own-delivery-mechanism) consequence 1, [12 §4.5](12-collaboration.md#45-the-local-keyboard-racing-an-injected-answer)). `Resolving` carries the response so a crash can report what was lost. |
...
| `Deliverable` | [06 §5.2.1](06-agent-layer.md#521-deliverable--a-field-of-interaction-computed-by-the-normalizer) | `native \| synthetic { requires_token } \| none { reason }`. A **field of `Interaction`**, not of the `Open` state (06 makes that call; [D13](decisions.md#d13--synthetic-delivery-is-a-gated-transaction-never-a-bare-write)'s `Open { deliverable }` is shorthand). Computed once by the normalizer from the responder plus D16's per-card-type table. Remote answerability renders from this, never from mere openness. |
| `NotDeliverableReason` | [06 §5.2.1](06-agent-layer.md#521-deliverable--a-field-of-interaction-computed-by-the-normalizer) | `not_position_independent \| index_not_derivable \| inferred_responder_disabled \| no_responder`. Why a card is read-only remotely; shown verbatim on the surface. |
...
| `ViewMode` | [05 §4](05-session-model.md#4-attachment-detach-and-multi-client-viewing) | `Grid \| Blocks \| Transcript` — which surface a client attached with ([D14](decisions.md#d14--agent-sessions-get-a-transcript-surface-blocks-are-for-shell-work)). Neither `Blocks` nor `Transcript` receives PTY bytes; `Transcript` is fed by the agent event stream and is the mobile default for an agent session. |
...
| `Frame` | [07 §2.1](07-remote-protocol.md#21-the-trait) | Exactly two kinds: `Text` (JSON `ProtoMessage`) and `Binary` (24-byte header + payload, [07 §3.6](07-remote-protocol.md#36-binary-payloads)). |
...
| `Axis` | [17 §1.2](17-panes-and-layout.md#12-types) | `Columns \| Rows`. Names what is being divided rather than the direction of the divider, which is ambiguous in every multiplexer's documentation. Used by 05 §2 and by 10 §9.1's launch YAML, which writes `columns`/`rows` and also reads the `horizontal`/`vertical` that tmux and other terminals files use. |
...
| `Target` | [04 §8.3](04-terminal-core.md) | `Url(Url) \| Path { raw, line, col } \| GitRef \| Issue { raw, repo } \| Custom { rule, slots: SmallVec<[(Slot, Range<u16>); 4]> }`. 04 is authoritative; [18 §2.6](18-semantic-open.md#26-the-match-and-target-types) reproduces it verbatim and records why each earlier divergence was resolved against 04. |
...
`Grid | Blocks | Transcript`) are **different concepts with confusingly similar names**.
