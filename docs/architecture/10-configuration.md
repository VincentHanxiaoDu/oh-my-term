...
|---|---|
| **TOML** ✅ | Unambiguous scalar typing; comments survive round-trips with `toml_edit`; byte spans available for every key and value (`toml_edit`/`toml::Spanned`), which is what makes §5's caret diagnostics possible; flat section headers map cleanly onto a settings UI's page/section structure; already the format most Rust CLI users expect. the `settings.toml` and the single TOML file both landed here independently. |
| YAML | Indentation-significant, so machine edits from a settings UI are risky; the Norway problem and other implicit-typing traps; `serde_yaml` is unmaintained; span quality is worse. Still used where the ecosystem already is (themes from other terminals, workflows). |
...
The `OmtConfig` derive is the omt analogue of the `define_setting!` +
`inventory` registry (see [research/other terminals.md §5.3](../design/terminal-ux.md)). It walks
...
other terminals treats them as diagnostics-only, which is friendlier for forward
...
lesson from [research/other terminals.md §8](../design/terminal-ux.md).
...
**the YAML format** (schema in [research/other terminals.md §3.2](../design/terminal-ux.md)) —
...
| `name` absent | derived from the filename (kebab → Title Case), |
...
`omt theme import --dir ~/.other terminals/themes` bulk-imports and prints a table of what
...
- **Trigger** — `mod-mod-key`, modifiers from `ctrl|alt|shift|cmd|super` in any
 ...
 string: `"ctrl-b c"`. This is the other terminals convention and it is a good one.
...
- **`"none"`** unbinds, exactly as the `REMOVED_KEYBINDING_SERIALIZATION`.
...
by name. The recursive pane-tree shape is adopted from the launch
configurations ([research/other terminals.md §5.2](../design/terminal-ux.md)) because it maps
...
windows:
 ...
 # never by typing into a shell (see research/other terminals.md §8).
 agent:
 id: claude_code
...
- **`split` is `columns` | `rows`**, matching the `Axis` type
 ...
 `vertical` that tmux, other terminals and hand-written files use **are accepted on read**
...
A workflow is a saved, parameterized command. the format
([research/other terminals.md §4](../design/terminal-ux.md)) is adopted deliberately unchanged
for the command variant, so the public `other terminals/workflows` corpus loads as-is:
...
omt extensions, all optional and ignored by the parser:
...
`agent.prompt` instead of the PTY — the equivalent of the `AgentMode`
...
| Importers | fixture corpus of 20 the YAML format + 20 `.itermcolors` files, golden output |
...
- **OPEN QUESTION — workflow argument types beyond `text`/`enum`.** other terminals has
