...
Research inputs: [iTerm2](../research/iterm2.md), .
...
|---|---|---|
| Vendor `ghostty-vt` (as other terminals does) | **Rejected** | Licence is fine (MIT) but it is Zig: a Zig toolchain in the build, an FFI boundary in the hottest loop, and a foreign memory model for the grid. other terminals accepts that because it only needs a screen scrape; omt must own the grid, marks and blocks in Rust to do reflow and block tracking. A Rust grid over a Zig parser is the worst of both. |
...
Licence note: we may read iTerm2 (GPL-2.0) and other terminals (AGPL-3.0) to learn *what
...
- **Historical block re-wrap is lazy.** Following the pragmatic choice, only
...
an omt `Block` **does not own grids**. It owns metadata plus a
range of positions into the one shared scrollback. the three-grid design is
tied to other terminals owning the input line; omt runs the user's real shell in a real PTY
...
**Attribution** is what makes blocks worth more in omt than elsewhere. When the
...
What the heuristic explicitly does **not** do: match prompt regexes. other terminals and
...
### 6.5 Interface facts borrowed from other terminals
...
[14](14-licensing.md)): from the other terminals study we take **only observations about
...
We do **not** take the three-grid block, its sum-tree, its OSC 9278 JSON hook
