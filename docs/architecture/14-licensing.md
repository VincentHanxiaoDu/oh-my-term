...
## 2. The codebases we studied, and their licences

| Project | Upstream | Licence | What we took |
|---|---|---|---|
| another terminal | `a terminal vendor/another terminal` | **AGPL-3.0**, except the `a vendor UI crate` and `a vendor UI crate` crates which are **MIT** | interface facts only: shell-integration escape-sequence numbers and payload shapes, theme YAML schema, workflow YAML schema, keybinding file grammar, launch-configuration schema, settings-registry *pattern* |
| iTerm2 | `gnachman/iTerm2` | **GPL-2.0** | interface facts only: VT/xterm escape-sequence coverage, DSR/DECRQM behaviour, the `.itermcolors` property-list key names |
| another tool | `another tool/another tool` | **Apache-2.0** | interface facts and, where useful, ideas — its plugin manifest shape, its detection-manifest concept, its config-diagnostic approach. Apache-2.0 is compatible with ours, but we still do not copy code; see §3 |
| Coding-agent CLIs | various | various | documented hook payload schemas, ACP/app-server protocol shapes, transcript file formats |

The research notes for each live in [`docs/research/`](../research/) and are
committed. The *checkouts* do not — see §5.

The AGPL and GPL entries are the ones that constrain us. Nothing in omt is
derived from AGPL or GPL code, so omt can be Apache-2.0. The MIT `a vendor UI crate` crates
would be compatible in principle, but we take nothing from them either: omt uses
ratatui, not a GPUI-derived UI stack.
...
- Escape-sequence numbers and their grammar (OSC 133, OSC 52, OSC 7, OSC 1337,
  kitty graphics, another terminal's OSC 9277/9280 payload framing, DEC private modes).
- Wire protocols and message shapes (ACP, agent app-server methods, hook payload
  JSON, SSE event names).
- File formats and schemas (another terminal's theme YAML keys, workflow YAML keys,
...
3. **The two roles are separated in time, and preferably by person.** For the
   AGPL and GPL targets (another terminal, iTerm2) this separation is the point. For
   Apache-2.0 targets (another tool) it is good hygiene that keeps the provenance story
   uniform.
4. **Every generated artifact is ours.** Themes imported from another terminal YAML are
   parsed by our parser into our format; we ship no another terminal code and no another terminal theme
...
- A CI job fails if any path under `.research/` is tracked, or if a commit adds
  a file matching known upstream markers (`a vendor core crate`, `a vendor UI crate`, `iTerm2.xcodeproj`,
  `another tool-plugin.toml` outside a test fixture).
...
- `NOTICE` at the repo root lists: omt's copyright, the crates whose licences
  ...
  files, detection manifests), and an "interfaces studied" section naming another terminal,
  iTerm2 and another tool **as interface references**.
...
- Naming another terminal or iTerm2 in `NOTICE` is a courtesy and a provenance record. It is
...
- Trademarks are not licensed: omt does not use the another terminal or iTerm2 names or
...
>       `a terminal vendor/another terminal`, `gnachman/iTerm2`, or any GPL/AGPL source.
...
[research/another terminal.md §2.4](../research/another terminal.md)" is exactly right.
...
- **OPEN QUESTION — bundling a community theme corpus.** `a terminal vendor/themes`
