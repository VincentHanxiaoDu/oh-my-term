...
│ "command": "other terminals-hook pre" } ] }, │
│ + { "matcher": "*", "hooks": [ { "type": "command", │
...
│ } │
│ │
│ ! other terminals is already hooked on PreToolUse. Both will run; omt does not remove │
...
2. **Does the palette's alias injection from §5.1's table scale?** Generating
 ...
 input. Fine for tmux (44 rows); unclear whether zellij, screen and other terminals
