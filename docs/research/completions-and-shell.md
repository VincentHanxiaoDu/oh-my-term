...
Companion to [`other terminals.md`] (§2 shell integration, §7 completions engine, §8 input/history)
and [`../architecture/04-terminal-core.md`](../architecture/04-terminal-core.md) §6–7. Those two
cover the approach; this document covers the rest of the landscape and ends with a build/buy
recommendation.
...
**Schema.** the v2 signature schema (`other terminals.md` §7.2) is the Fig schema with other terminals-specific
...
declarative and cheap; `custom` and `postProcess` are functions. other terminals solved this by embedding a JS
engine and shipping compiled specs as an embedded asset.
...
- `onBufferChange` sets `#suspended = buffer.type === "alternate"` — **all command tracking is
 disabled on the alt screen**, matching the rule and omt's 04 §6.3.
...
This is what other terminals does over OSC 9280 (`other terminals.md` §7.4) with the shell it already owns. An external
...
4. **the OSC 9280 approach avoids all of this** by asking the *already-running, already-warm,
 already-configured* shell — at the cost of needing a private protocol and a shell that is
 sitting at a prompt. `other terminals.md` §7.4's `IFS= read -d $'\4' -s "$(echo -e "TEMP?\e]9280;P\a")"
...
- `k=<kind>` — prompt kind for `133;P`: `i` = initial/primary prompt, `s` = secondary (PS2),
 ...
 other terminals uses `\e]133;P;k=r\a` for the right prompt `[SRC] other terminals.md §2.4`.
...
**Emitters**: kitty, Ghostty, WezTerm, iTerm2, VS Code, other terminals, inshellisense (as a secondary),
...
tmux 3.4+ (partial), other terminals (prompt-region routing only — it uses its own hooks for lifecycle).
...
| `OSC 9278`, `9277`, `9279`, `9280` | other terminals | JSON hooks, in-band generators, grid reset, completions `[SRC] other terminals.md §2.1` |
...
is the one other terminals uses: *detect existing `\e]133;A` in `PS1`/`PROMPT` and do not add a second*
`[SRC] other terminals.md §2.4` — plus kitty's strip-then-re-add idempotence `[SRC]`.

**bash-preexec** (MIT) — the universal answer to "bash has no `preexec`". It hooks `DEBUG` trap +
`PROMPT_COMMAND`. Vendored `[SRC]` and by inshellisense `[SRC] shell/bash-preexec.sh`,
and named in omt's `04 §7.1`. Its documented limitation is in the own comment `[SRC] other terminals.md
...
| Case | kitty | Ghostty | VS Code | iTerm2 | other terminals | **omt (04 §7.3)** |
|---|---|---|---|---|---|---|
...
| **(b) subshells** | env vars inherit; `KITTY_SHELL_INTEGRATION` re-read | same | same | the `source` line is in the rc, so any new shell re-runs it | env inherit + `*_init_subshell.sh` assets + a synthetic "other terminals" block | env inherit (`OMT_SESSION`, `OMT_SHELL_INTEGRATION`, nonce) |
...
the session `[DOC]`. That is a genuinely better answer than the rc-append, and it is worth
...
tty. the `commands` table `[SRC] other terminals.md §8.2` has `git_branch`, `start_ts`/`completed_ts` as
separate columns, `username`, `shell`, `workflow_command`, `is_agent_executed`. the is closer to
what omt wants.
...
so the scan is early-terminating. other terminals does the equivalent in Rust with a reverse walk and two
independent `HashSet`s — one for commands, one for AI prompts `[SRC] other terminals.md §8.2`.
...
`[INF]` — a merge of the columns, atuin's discipline, and omt's own `Attribution`:
...
**other terminals** highlights the input line because other terminals *is* the line editor — `InputBufferModel { buffer_value, cursor_point }` `[SRC] other terminals.md §8.1`, with colour derived from the completer's own token
classification (`Command→Green, Subcommand→Blue, Variable→Magenta, Argument→Cyan, Option→Yellow`).
the shell runs with its line editor effectively bypassed.
...
3. **Post-hoc highlighting of the *command* in a closed block.** Once `133;C` fires and the command
 ...
 coloured however omt likes. **This delivers most of the visual signature for none of the
...
channel. This is exactly the OSC 9280 pattern inverted. It works, it is the only way to get real
...
- Every other terminals-class polish item becomes available: inline highlighting, ghost-text autosuggestion,
...
- Typeahead capture stops being a problem to solve (`other terminals.md` §2.4's `ESC-i` `InputBuffer` hook
 exists only because other terminals owns the line).
...
- **It breaks the user's shell.** Every zsh user with `zsh-autosuggestions`,
 ...
 loses all of it. That is not a minority; on zsh it is close to the majority. other terminals can absorb this
 because other terminals is a product with a marketing budget; an open-source terminal that silently disables
...
- **It requires bypassing the shell's line editor**, which means either running the shell
 ...
 or fighting ZLE for the buffer. other terminals does the former for its own generators via a private channel
 and still keeps a real interactive shell for the user; the complexity in `other terminals.md` §2.5 and §8.1
 is the price.
...
- inshellisense — the project that tried hardest to get other terminals-class autocomplete without being a
 ...
 *is* the input. On desktop, the composer is one keystroke away and is where the good UX lives.
 A user who wants the experience opens the composer; a user who wants their dotfiles types at
...
- **Do not embed a JavaScript engine.** Terminals that have done this shipped a JS runtime plus compiled specs as an embedded asset, and it
...
3. **Idempotence and coexistence, non-negotiable.** Before adding marks, strip omt's own marks from
 ...
 prompt variables; if present, do not add a second set** . Detect
...
5. **ssh**: prefer kitty's approach — `omt ssh` pushes a self-contained emitter into the remote's
 ...
 nothing behind. Fall back to the rc-append **only with explicit per-host confirmation**, as
